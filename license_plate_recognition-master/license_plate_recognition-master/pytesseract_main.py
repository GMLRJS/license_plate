...
# "carset" 폴더에 있는 모든 파일을 불러옵니다.
for filename in os.listdir('carset'):
    # 파일의 확장자를 확인하여 이미지 파일인지 검사합니다.
    if filename.endswith('.png') or filename.endswith('.jpg') or filename.endswith('.jpeg'):
        ...
        for i, plate_img in enumerate(plate_imgs):
            ...
            #pytesseract를 이용하는 부분
            #oem 설정 다르게 해줘야 될 수도 있음. 기본은 0 이었는데, oem = 0 일때가 lagacy인 경우 경로문제가 발생할 수 있다.
        ...
        # cv2.imwrite(chars, img_out)
        #뒷번호 4자리만 남기는 설정(한글 인식 문제)
        if len(chars)==7 or 8:
            chars = chars[len(chars)-4:len(chars)]
        print(chars)

        imwrite("result_tesseract/"+chars+'.jpg',img_out)

        # 이미지 파일이 정상적으로 읽혔는지 확인합니다.
